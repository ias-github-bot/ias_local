...
class ParticularReport(models.AbstractModel):
	...
	def render_html(self, cr, uid, ids, data=None, context=None):
		...
		from_date_x = self.pool.get('hr.attendance.reporting').browse(cr, uid, context.get('active_id'),
																	  context=context).date_from
		to_date_x = self.pool.get('hr.attendance.reporting').browse(cr, uid, context.get('active_id'),
																	context=context).date_to
			...
			for object in docs:
				if object.action == 'sign_in':
					...
					if date == att_date:
						...
						attendance_time = pytz.utc.localize(datetime.strptime(attendance_datetime,"%Y-%m-%d %H:%M:%S"
																			  )).astimezone(tz)
		...
		def _get_diff_time(docs,sign_in_time,date):
			...
			if sign_in_time == "Not Found":
				...
				for wk in employee.contract_id.resource_calendar_id.attendance_ids:
		...
		def _get_overtime(docs,date):
			...
			employee = self.pool.get('hr.employee').browse(cr, uid, employee_id, context=context)
			day_ids = []
			for day in employee.contract_id.resource_calendar_id.attendance_ids:
			...
			for object in docs:
				...
				if att_date.date() == date:
					hour_to = get_end_hour_of_the_day(att_date, employee.contract_id.resource_calendar_id.id)
					...
					if employee.contract_id.overtime_structure_id.overtime_method == 'ov_attendance':
						# get Official Leave
						holiday_status_id = self.pool.get('hr.holidays.status').search(
							cr, uid,[('name','=','Official Leave')], context=context)[0]
						domain =[
							('employee_id','=',object.employee_id.id),
							...
							if self._get_index_day(att_date.date()) in day_ids:
								if Flage == False:
									if rule.type == 'working_day' and object.action == 'sign_out':
										...
										start_overtime = tz.localize(datetime.combine(att_date.date(),
																					  start_overtime_time))
						...
						for rule in employee.contract_id.overtime_structure_id.hr_ov_structure_rule_ids:
							ov_ids = self.pool.get('hr.overtime').search(
								cr, uid, [('employee_id','=',employee.id),('state','=','approve')], context=context)
							for overtime in self.pool.get('hr.overtime').browse(cr, uid, ov_ids, context=context):
								ov_date = pytz.utc.localize(datetime.strptime(overtime.from_date,"%Y-%m-%d %H:%M:%S"
																			  )).astimezone(tz)
